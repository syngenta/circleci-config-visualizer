...
import { getGithubData } from "../../redux/githubData/githubDataSlice";
...
export default function Home() {
  ...
  const [viewGithubWindow, setViewGithubWindow] = useState(false);

  return (
    <>
      {viewGithubWindow && <GitHub viewGithubWindow={viewGithubWindow} setViewGithubWindow={setViewGithubWindow} />}
        ...
        >
          <div className="flex flex-col justify-start items-start">
            ...
            <p className="text-[15px] text-gray-700 w-[500px] mt-4 dark:text-gray-400">
              Gain a comprehensive visual understanding of your CircleCI
              pipelines with an intuitive and interactive config visualizer
            </p>
            <div className="flex flex-row justify-center items-center gap-6 mt-6">
              <PrimaryButton
                icon={<FaGithub />}
                label={githubData?.token?`Load from GitHub`:`Connect GitHub`}
                className="px-4"
                color="bg-gray-700"
                onClick={()=>{setViewGithubWindow(true)}}
              />
              <input
                type="file"
                id="fileInput"
